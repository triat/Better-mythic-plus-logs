...
describe("user settings repository", () => {
  test("defaults before any write", () => {
    const { s, ua } = setup();
    expect(s.get(ua.id)).toEqual({ yourKey: null, legendOpen: true, region: null, locale: null });
    expect(DEFAULT_USER_SETTINGS).toEqual({ yourKey: null, legendOpen: true, region: null, locale: null });
  });

  test("update merges a partial patch and returns the whole row; users are independent", () => {
    const { s, ua, ub, db } = setup();
    expect(s.update(ua.id, { yourKey: 18 }, 1000)).toEqual({ yourKey: 18, legendOpen: true, region: null, locale: null });
    expect(s.update(ua.id, { legendOpen: false }, 2000)).toEqual({ yourKey: 18, legendOpen: false, region: null, locale: null });
    expect(s.update(ua.id, { yourKey: null }, 3000)).toEqual({ yourKey: null, legendOpen: false, region: null, locale: null });
    expect(s.get(ub.id)).toEqual({ yourKey: null, legendOpen: true, region: null, locale: null });
  ...
  test("region is stored and read back; an unrecognised value read from the row falls back to null", () => {
    const { s, ua, db } = setup();
    expect(s.update(ua.id, { region: "kr" }, 1000)).toEqual({ yourKey: null, legendOpen: true, region: "kr", locale: null });
    expect(s.get(ua.id)).toEqual({ yourKey: null, legendOpen: true, region: "kr", locale: null });
    expect(s.update(ua.id, { region: null }, 2000)).toEqual({ yourKey: null, legendOpen: true, region: null, locale: null });
  ...
  test("locale is stored and read back; an unrecognised value read from the row falls back to null", () => {
    const { s, ua, ub, db } = setup();
    expect(s.update(ua.id, { locale: "fr" }, 1000)).toEqual({ yourKey: null, legendOpen: true, region: null, locale: "fr" });
    expect(s.get(ua.id)).toEqual({ yourKey: null, legendOpen: true, region: null, locale: "fr" });
    expect(s.update(ua.id, { locale: null }, 2000)).toEqual({ yourKey: null, legendOpen: true, region: null, locale: null });
