...
- **Deployment** (issue #10): `test/deploy.test.ts` reads the `deploy/` files as text and pins what `deploy/README.md` relies on — `reverse_proxy 127.0.0.1:3000` and HSTS in the Caddyfile (no `trusted_proxies`), the unit's `ExecStart`/`User=bmpl`/hardening directives, `path: /opt/bmpl/bmpl.db` and placeholder-only credentials in `litestream.yml`, the hourly timer and `/usr/local/sbin/bmpl-backup-check`, the ufw ports 22/80/443 (never 3000) in `bootstrap.sh`, and that no real secret or host leaks into a template. Change a path in `deploy/` → change the test and `deploy/README.md` in the same commit. Nothing in the suite ever reaches a VPS; `test/cli-serve.test.ts` covers `planServe()`'s host resolution and `test/server-hosted.test.ts` that `runServer` binds the host it is given.
