...
# `litestream snapshots` prints one row per snapshot; the creation time is the last column.
newest=$(litestream snapshots -config /etc/litestream.yml /opt/bmpl/bmpl.db | awk 'NR > 1 { print $NF }' | sort | tail -n 1)
