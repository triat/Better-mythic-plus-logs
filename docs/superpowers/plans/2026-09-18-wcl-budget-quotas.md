...
- `PointsMeter` (`src/wcl/meter.ts`): `run(userId, fn)` scopes attribution with AsyncLocalStorage; `observe(rl)` charges `delta` = counter minus the previous observation of the same WCL window (`windowEnd = observedAt + pointsResetIn * 1000`; a new window opens only when the observation is at/past `windowEnd` *and* the counter dropped, in which case the whole counter is the delta — otherwise, whatever the clock says, it's an in-window delta and `windowEnd` is refreshed from that response; a lower counter before `windowEnd` charges 0 and keeps the higher counter; a delta above `MAX_CHARGE_PER_OBSERVATION` (100) is a likely clock-boundary artefact and re-primes the baseline instead of charging); the first observation only primes (charged to nobody); charges are rounded to 0.1 pt; charges outside `run` are not attributed; `wrap(gql)` returns a `GqlFn` that observes its responses. Cost estimates: `ESTIMATE_RANKINGS = 10`, `ESTIMATE_RUN = 10`, `ESTIMATE_DEEPDIVE = 3`.
