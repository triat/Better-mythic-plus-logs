...
export const DISCORD_GUILDS_URL = "https://discord.com/api/users/@me/guilds";

export type FetchFn = typeof fetch;

export const redirectUri = (baseUrl: string): string => `${baseUrl}/auth/discord/callback`;

export function authorizeUrl(cfg: { discordClientId: string; baseUrl: string; discordGuildId?: string | null }, state: string): string {
  const u = new URL(DISCORD_AUTHORIZE_URL);
  u.searchParams.set("client_id", cfg.discordClientId);
  u.searchParams.set("response_type", "code");
  u.searchParams.set("scope", cfg.discordGuildId ? "identify guilds" : "identify");
...
/** Guild ids the account belongs to, used only for the `BMPL_DISCORD_GUILD_ID` gate; nothing from the response is stored. */
export async function fetchDiscordGuilds(
  accessToken: string,
  fetchFn: FetchFn,
): Promise<{ ok: true; guildIds: string[] } | { ok: false; error: string }> {
  try {
    const res = await fetchFn(DISCORD_GUILDS_URL, { headers: { Authorization: `Bearer ${accessToken}` } });
    if (!res.ok) return { ok: false, error: `Discord /users/@me/guilds answered ${res.status}` };
    const j = (await res.json().catch(() => null)) as unknown;
    if (!Array.isArray(j)) return { ok: false, error: "Discord /users/@me/guilds payload is not a list" };
    const guildIds = j.filter((g): g is { id: string } => !!g && typeof g === "object" && typeof (g as { id?: unknown }).id === "string").map((g) => g.id);
    return { ok: true, guildIds };
  } catch (e) {
    return { ok: false, error: `Discord /users/@me/guilds request failed: ${message(e)}` };
  }
}
