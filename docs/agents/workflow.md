...
Sub-projects 1–4 shipped (signals, evaluation model, web front, run deep-dive). Next: sub-project 5, hosted multi-user service — fully specified in GitHub issue #1 and its sub-issues; #2–#4 (hosted skeleton, Discord login, per-user state) are shipped; execute #5 → #11 in order, each keeping local mode green. Known follow-ups not yet scheduled: user validation of `defensives.json` cooldowns per spec; choice-node ("either/or") entries and talent-aware cooldowns via `combatantinfo`; absolute per-level references for peer signals; a `performLookup` end-to-end test once `fetchMplusData` is injectable; in-flight dedupe in `enrichRuns`; evaluation `expectedIlvl` values unvalidated against real profiles.
