    ...
    <div className="inset sub-card" id={`${axis}.${id}`}>
      <div>
        <div className="sub-head">
          ...
          {doc.scaledByLevel && <a href="#level-scale" className="chip chip-warn">{t("help.scaledByLevel")}</a>}
        </div>
        <div className="kv">
          <span className="k">{t("help.kv.what")}</span><span>{doc.what}</span>
          <span className="k">{t("help.kv.source")}</span><span>{doc.source}</span>
          <span className="k">{t("help.kv.how")}</span><span>{doc.how}</span>
          <span className="k">{t("help.kv.why")}</span><span>{doc.why}</span>
          <span className="k">{t("help.kv.na")}</span><span>{doc.naWhen}</span>
        </div>
      </div>
      <div className="curve">
        <div className="label-caps">{t("help.curve.caption", { unit: doc.unit })}</div>
        ...
        <div className="weights">
          {roleWeights(cfg.weights).map((w) => <span key={w.role} className="chip">{t("verdict.weight.each", { role: t(`verdict.role.${w.role}`), w: w.weight })}</span>)}
        </div>
      </div>
    </div>
  );
