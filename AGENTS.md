...
src/hosted/           hosted-mode config, schema/repos (users, sessions, invites), cookie/state/Discord helpers, auth gate
...
Current state and roadmap: sub-projects 1–4 (signals, evaluation, web front, deep-dive) are shipped. Sub-project 5 (hosted multi-user service) is filed as GitHub issues #1–#11; #2 and #3 (hosted skeleton, Discord login) are shipped, next is #4 (per-user state).
