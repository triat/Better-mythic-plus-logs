...
| `BMPL_ENCRYPTION_KEY` | `openssl rand -base64 32`, edit `.env`, restart `bmpl` | every previously stored member WCL client becomes undecryptable, but silently: `credentials()` returns `null` for the row and logs `wcl client of user N cannot be decrypted (key changed?)` (`src/hosted/wcl-clients.ts:117-126`), so the shared-vs-own scope check falls back to the shared client (`src/server/deepdive.ts:25-26`) — no request fails, and until they re-save their client, their lookups spend the shared budget and their own quota instead of their client. The member is told: the user menu reads "Your WCL client needs re-saving · using the shared budget" and the Settings card "Stored secret cannot be decrypted (the instance key changed) — save the client again" (`OwnClientView.usable`, `wcl-clients.ts`); saving it there fixes that one member |
...
| A member's own WCL client saved fine, but their lookups count against the shared budget and their per-member quota; the user menu reads "Your WCL client needs re-saving · using the shared budget" | `BMPL_ENCRYPTION_KEY` was rotated since they saved it: their row is now undecryptable, `credentials()` returns `null` and logs `wcl client of user N cannot be decrypted (key changed?)` (`src/hosted/wcl-clients.ts:117-126`), and `wclScopeFor` falls back to the shared client (`src/server/deepdive.ts:25-26`) | have them open Settings — the card says "Stored secret cannot be decrypted (the instance key changed) — save the client again" — and re-save the client |
...
| Locked out of SSH from one network after a few typos | fail2ban banned that IP for an hour (5 failures in 10 min); `journalctl -u fail2ban` names it | from another network or the provider's console: `fail2ban-client set sshd unbanip <ip>` |
| The VPS rebooted in the night | unattended-upgrades installed a security update that needed one (04:00 UTC); every unit is enabled, so bmpl is back by itself | `journalctl -u unattended-upgrades -b -1` says which package; nothing to do |
