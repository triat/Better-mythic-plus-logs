...
export const EVALUATION_DOCS_FR: EvaluationDocs = {
  ...
  verdict: {
    summary: "Le verdict est une lecture par règles de six axes. Chaque axe est noté de 0 à 100 à partir de ses sous-signaux ; le score global situe la moyenne pondérée des axes parmi de vrais joueurs du même rôle ; le verdict est le score global face à deux seuils.",
    ...
    global: "D'abord la moyenne pondérée : Σ(poids de l'axe × score de l'axe) / Σ(poids de l'axe) sur les axes qui ont pu être notés, avec les poids du rôle du joueur. Un axe n/a est exclu des deux sommes, donc il n'aide ni ne pénalise. Ensuite cette moyenne est située parmi un millier de joueurs EU qui loguent des clés +15 à +20 dans le même rôle, et le score global est ce percentile, arrondi : 50 est le joueur typique de cette population, 90 fait mieux que neuf sur dix d'entre eux. Ce placement ne change jamais qui passe devant qui ; il étale seulement l'échelle, qui serrait sinon presque tout le monde entre 60 et 90.",
    drivers: "Ce qui fait ce score : pour chaque signal, combien de points du score global le joueur gagne ou perd face au joueur moyen du rôle — la médiane d'environ un millier de joueurs EU qui loguent des clés +15 à +20. −19 sur les morts veut dire que mourir aussi souvent que le joueur moyen, tout le reste égal, rapporterait 19 points. La médiane plutôt que la moyenne parce que les morts et les dégâts évitables sont déséquilibrés : quelques joueurs à trois morts par run tirent une moyenne vers le haut, et « moyen » voudrait alors dire pire que la plupart. Les effets ne s'additionnent pas exactement au score, parce que le global est un percentile, pas une somme. La dernière ligne cite les plus gros coûts, trois au plus, qui ensemble suffiraient pour atteindre le verdict suivant s'ils étaient au niveau du joueur moyen. Les signaux du deep-dive n'ont pas encore de moyenne et sont laissés de côté.",
