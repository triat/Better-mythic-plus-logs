...
- **View models are pure and tested** (`web/src/lib/*.ts` + sibling `*.test.ts`): every non-trivial transformation of `LookupPayload` into something displayable lives there — `verdict.ts` (`AXIS_DESCRIPTIONS`, `AXIS_WEIGHTS`, confidence colours), `axes.ts` (`AxisRowModel`), `radar.ts`, `tiles.ts`, `runs.ts`, `deepdive.ts` (panel model, cost estimate), `compare.ts`, `history.ts`, `keyLevel.ts`, `format.ts`, `session.ts` (user menu / quota line), `hostedMode.ts` (controls, `proposalMode`), `quota.ts` (labels, tooltips, 429 bodies). Components (`web/src/components/*.tsx`) stay thin: map a model to markup, hold local UI state only. Tests must not import React or the DOM (they run under `bun test` from the repo root).
...
`Home` (empty state) · `Header` (search form, level/spec/metric, watch toggle, quit; hosted: `UserMenu` — avatar, quota, Admin, Sign out) · `SignIn` (hosted gate) · `Tabs` · `Detail` = `VerdictHero` (verdict, radar, `AxisRows` with expandable "What's measured" callouts, `AxisLegend` under the radar) + `SignalTiles` + `DungeonRuns` (per-run Analyze / `RunDeepDive` panel with table corrections; hosted wording and proposals footer) + `RioSection` · `Compare` (2–3 tabs) · `Setup` (credentials) · `Toast` · `KeyStepper` · `Radar` · `SpellLink`.
