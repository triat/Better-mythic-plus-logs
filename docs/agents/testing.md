...
`bun test` from the repo root runs everything: `test/**/*.test.ts` (backend) and `web/src/**/*.test.ts` (front view models). ~275 tests, a few seconds, no network, no `web/dist` needed. `just check` (tsc for both projects) is the other gate. Both must be green before a commit.
...
- **Server tests** (`test/server*.test.ts`) start `runServer({ port: 0, … })` on an ephemeral port with a temp dir / in-memory store and fake `gql`; they must pass with `web/dist` missing (static routes answer 503 — `test/server.test.ts` covers it).
