...
- **Wowhead tooltips**: `web/index.html` loads `https://wow.zamimg.com/js/tooltips.js` with `whTooltips = { colorLinks:false, iconizeLinks:false, renameLinks:false }`; render spell names through `SpellLink.tsx` (`useWowheadRefresh` calls `$WowheadPower.refreshLinks()` after mount). Any new external origin must be justified — the planned hosted CSP allows only `wow.zamimg.com` besides self.
